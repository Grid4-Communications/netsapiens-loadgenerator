...
/**
 * Parse new lines from a file starting from a specific position
 * This allows incremental parsing without re-reading the entire file
 * @param {string} filePath - Path to the file
 * @param {number} startPos - Byte position to start reading from (0 for header)
 * @returns {Object} { lines: Array<Object>, newSize: number, header: string }
 */
function parseIncrementalLines(filePath, startPos = 0) {
  try {
    const fd = fs.openSync(filePath, 'r');
    const stats = fs.fstatSync(fd);
    const fileSize = stats.size;
    ...
    if (fileSize === 0 || fileSize <= startPos) {
      fs.closeSync(fd);
      return { lines: [], newSize: fileSize, header: null };
    }

    // If starting from beginning, read header first
    let header = null;
    let dataStartPos = startPos;
    ...
    if (startPos === 0) {
      ...
      header = headerText.substring(0, headerEnd);
      dataStartPos = headerEnd + 1; // Start after header
    }

    // Read new data from startPos to end of file
    const bytesToRead = fileSize - dataStartPos;
    if (bytesToRead <= 0) {
      fs.closeSync(fd);
      return { lines: [], newSize: fileSize, header };
    }

    // Cap reading to 1MB at a time to avoid memory issues
    const maxRead = Math.min(bytesToRead, 1024 * 1024);
    const dataBuffer = Buffer.alloc(maxRead);
    fs.readSync(fd, dataBuffer, 0, maxRead, dataStartPos);
    fs.closeSync(fd);
    ...
    for (const line of dataLines) {
      try {
        ...
        if (records.length > 0) {
          parsedLines.push(parseStatsRecord(records[0]));
        }
      } catch (e) {
    ...
    return {
      lines: parsedLines,
      newSize: dataStartPos + maxRead,
      header
    };
  } catch (error) {
    console.error(`Error reading incremental lines from ${filePath}:`, error.message);
    return { lines: [], newSize: startPos, header: null };
  }
}
