...
function readHeaderAndLastLine(filePath) {
  try {
    ...
    // Read first 2KB to get header (should be more than enough for one line)
    const headerBuffer = Buffer.alloc(Math.min(2048, fileSize));
    ...
    // Read last 8KB to get the last line (CSV lines can be long with many columns)
