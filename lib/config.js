...
class ConfigLoader {
  ...
  _loadMultiServerConfig(filePath) {
    try {
      ...
      this.servers = config.servers.map(server => ({
        ...
        peakCps: server.peakCps || parseInt(process.env.PEAK_CPS) || 10,
        registrationPct: server.registrationPct || parseInt(process.env.REGISTRATION_PCT) || 80,
        seed: server.seed || parseInt(process.env.SEED) || Math.floor(Math.random() * 100000),
  ...
  _loadSingleServerConfig() {
    ...
    const server = {
      ...
      peakCps: parseInt(process.env.PEAK_CPS) || 10,
      registrationPct: parseInt(process.env.REGISTRATION_PCT) || 80,
  ...
  _validate() {
    ...
    if (this.selectedServer) {
      ...
      if (this.selectedServer.peakCps < 1) {
        throw new Error('peakCps must be at least 1');
      }
      if (this.selectedServer.registrationPct < 0 || this.selectedServer.registrationPct > 100) {
        throw new Error('registrationPct must be between 0 and 100');
      }
    }
  }
