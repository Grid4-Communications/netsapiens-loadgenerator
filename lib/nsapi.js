...
// Legacy mode - environment variables (backward compatibility)
...
/**
 * ServerApiClient - Multi-server API client
 * Encapsulates API calls to a specific NetSapiens server
 */
class ServerApiClient {
    constructor(serverConfig) {
        ...
        // Create server-specific axios instance with connection pooling
     ...
     * @private
     */
    async _retryApiCall(apiCall, retries = RETRY_CONFIG.maxRetries) {
        try {
            ...
            if (retries > 0 && isRetryable) {
                ...
                if (this.debug) {
                    console.log(`[${this.serverId}] Retrying API call in ${delay}ms. Retries left: ${retries}. Error: ${error.message}`);
                }
     ...
     */
    async apiUpdate(path, data, successFunction) {
        if (typeof successFunction !== 'function') successFunction = () => { };
        const url = `https://${this.hostname}/ns-api/v2/${path}`;
        ...
        const makeRequest = async () => {
            ...
            if (this.debug) console.log(`[${this.serverId}]`, options.method, url, data);
            return this.axiosInstance.request(options);
        };

        try {
            const response = await this._retryApiCall(makeRequest);
            if (this.debug) console.log(`[${this.serverId}] Response`, response.status, response.statusText);
            successFunction(data);
        } catch (error) {
            console.error(`[${this.serverId}] PUT ERROR - ${url}`);
    ...
    /**
     * Create a new resource (POST) - async, doesn't wait for response
     */
    async apiCreate(path, data, successFunction, duplicateFunction) {
        if (typeof successFunction !== 'function') successFunction = () => { };
        ...
        try {
            ...
            if (error.response && error.response.status == 409) {
                if (this.debug) console.log(`[${this.serverId}] Response`, error.response.status, error.response.statusText);
                ...
                console.error(`[${this.serverId}] POST ERROR url - ${url}`);
                if (error?.response?.status) {
                    console.error(`HTTP ${error.response.status}: ${error.response.statusText}`);
                    if (error.response.data) {
                        console.error("Response data:", error.response.data);
    ...
    /**
     * Create a new resource (POST) - synchronous, waits for response
     */
    async apiCreateSync(path, data, successFunction, duplicateFunction) {
        if (typeof successFunction !== 'function') successFunction = () => { };
        if (typeof duplicateFunction !== 'function') duplicateFunction = () => { };
        const url = `https://${this.hostname}/ns-api/v2/${path}`;
...
module.exports = {
    ServerApiClient,  // New multi-server class
    apiUpdate,        // Legacy function for backward compatibility
    apiCreate,        // Legacy function for backward compatibility
    apiCreateSync     // Legacy function for backward compatibility
}
