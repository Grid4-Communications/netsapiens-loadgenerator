 ...
 * Tracks distribution of SIPp response times with predefined buckets
 */
const responseTimeHistogram = new client.Histogram({
  ...
  buckets: [0.01, 0.02, 0.03, 0.05, 0.1, 0.2, 0.5, 1, 2, 5], // 10ms to 5s
  registers: [register]
});

/**
 * Response Time Summary
 * Tracks response time summary statistics with quantiles
 */
...
function updateResponseTimeMetrics(serverId, scenario, operation, responseTimes) {
  ...
  // Update histogram and summary with individual samples
  // Note: We're approximating by setting observations based on distribution buckets
  // This is not perfect but gives us histogram data from SIPp's bucketed output
  if (responseTimes.samples && responseTimes.samples.length > 0) {
    // For more accurate histogram/summary, we'd need raw samples
    // As a compromise, we'll observe the percentile values
    if (responseTimes.percentiles.p50 > 0) {
      responseTimeHistogram.observe(labels, responseTimes.percentiles.p50);
      responseTimeSummary.observe(labels, responseTimes.percentiles.p50);
    }
    if (responseTimes.percentiles.p95 > 0) {
      responseTimeHistogram.observe(labels, responseTimes.percentiles.p95);
      responseTimeSummary.observe(labels, responseTimes.percentiles.p95);
    }
    if (responseTimes.percentiles.p99 > 0) {
      responseTimeHistogram.observe(labels, responseTimes.percentiles.p99);
      responseTimeSummary.observe(labels, responseTimes.percentiles.p99);
    }
  }
